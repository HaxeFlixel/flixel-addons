...
enum FlxGameJoltRequestType
{
	 ...
	 * @param   parallel      Whether to execute all `requests` at once (`true`) or at the
	 *                        respective order (`false`).
	 * @param   breakOnError  Whether to return a general error message if one of the requests fails.
	 ...
	 * @param   fromUser  Whether to retrieve data from the registered user (`true`) or from the game itself (`false`).
	 */
	DATA_FETCH(key:String, fromUser:Bool);
	
	/**
	 * Retrieves all the keys registered in the Data Store.
	 * @param   fromUser  Whether to retrieve keys from the registered user (`true`) or from the game itself (`false`).
	 * @param   pattern   An optional pattern to apply to the key names in the data store.
	 */
	 ...
	 * @param   fromUser  Whether to remove data from the registered user (`true`) or from the game itself (`false`).
	 */
	 ...
	 * @param   fromUser  Whether to set data to the registered user (`true`) or to the game itself (`false`).
	 */
	DATA_SET(key:String, data:String, toUser:Bool);
	
	/**
	 * Updates data in the Data Store.
	 * @param   key        The data's lookup key 
	 * @param   operation  The update operation to perform.
	 * @param   fromUser   Whether to update data for the registered user (`true`) or for the game itself (`false`).
	 */
	 ...
	 * @param   fromUser    Whether to retrieve only the score submitted by the registered user.
	 * @param   table_id    The score table ID to retrieve the score from. If `null`, they'll be
	 *                      retrieved from the primary score table of your game.
	 * @param   limit       How many scores to retrieve. Must be a value between 1-100, Default is 10.
	 * @param   betterThan  If you want to retrieve only the scores that are HIGHER than some value,
	 *                      you can pass such value here.
	 *                      **NOTE:** If you set a negative value, this will retrieve the scores that
	 *                      are LOWER than its value instead.
	 */
	...
	/**
	 * Retrieves information about all the trophies in your game, and their status according to
	 * the registered user.
	 * @param   achieved   Whether to retrieve only achieved (`true`) or unachieved (`false`) trophies.
	 *                     Leave `null` to retrieve them all.
	 * @param   trophy_id  If you want to retrieve an specific trophy, you can pass its ID here.
	 * If not `null`, `achieved` will be ignored.
	 */
